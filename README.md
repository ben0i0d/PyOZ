...
- **Automatic `.pyi` stub generation** - Type hints generated at compile time for IDE support
...
pyoz build --no-stubs  # Build without generating .pyi stubs
...
PyOZ automatically generates Python type stubs (`.pyi` files) at compile time, providing full IDE support with autocomplete, type checking, and documentation.

### How It Works

1. **Compile-time generation**: Stubs are generated during Zig compilation using comptime reflection
2. **Embedded in binary**: The stubs are embedded directly in the compiled `.so`/`.pyd` file
3. **Extracted at build time**: When you run `pyoz build`, stubs are extracted and included in the wheel
4. **Survives stripping**: Stubs are stored in a special binary section that survives `strip`
...
pub const Module = pyoz.module(.{
    ...
    .classes = &.{pyoz.class(Point)},
});
```

PyOZ generates:
```python
# mymodule.pyi - Auto-generated by PyOZ

def add(arg0: int, arg1: int) -> int:
    """Add two integers"""
    ...

def greet(arg0: str) -> str:
    """Greet someone"""
    ...
...
### Controlling Stub Generation

```bash
pyoz build              # Generate stubs (default)
pyoz build --stubs      # Explicitly enable stubs
pyoz build --no-stubs   # Disable stub generation
```

### Type Mappings

| Zig Type | Python Type Hint |
|----------|------------------|
...
| `?T` | `Optional[T]` |
| `[]T` | `list[T]` |
| `std.StringHashMap(V)` | `dict[str, V]` |
| `std.AutoHashMap(K, V)` | `dict[K, V]` |
| `pyoz.PyObject` | `Any` |
...
# strip = true              # Strip debug symbols (stubs still work!)
```
